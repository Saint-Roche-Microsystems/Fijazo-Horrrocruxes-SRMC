...
| **Tarjeta Kanban** | *(pendiente de enlazar)* |
...
| 3 | Ideas de nombre para el código de error del argumento inválido y para el caso de timeout. | Sugirió `BAD_REQUEST` / `VALIDATION_ERROR`, y una excepción de timeout propia. | Descarté las dos. `BAD_REQUEST` es vocabulario HTTP y este contrato no es HTTP: usé `INVALID_ARGUMENT`, el vocabulario de transporte con contrato que sigue el curso. Y no hice una excepción de timeout aparte: el repo ya trata timeout y socket caído como lo mismo (`UserValidationUnavailableError` en bets-service), porque al llamador le da igual la causa — lo que necesita saber es que no hay respuesta. Mantener esa simetría vale más que un nombre más preciso. |
